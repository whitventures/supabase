...
const SOC2 = () => {
  ...
  const fetchSOC2 = async (orgSlug: string) => {
    try {
      const soc2Link = await getDocument({ orgSlug, docType: 'soc2' })
      if (soc2Link?.link) window.open(soc2Link.link, '_blank')
    } catch (error: any) {
      ui.setNotification({
        category: 'error',
        ...
        <ScaffoldSectionContent>
          ...
          {isSuccess && (
            <div className="flex items-center justify-center h-full">
              {/* TODO: CHANGE ENTERPRISE AFTER TESTING */}
              {currentPlan?.id === 'enterprise' || currentPlan?.id === 'pro' ? (
